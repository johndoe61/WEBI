...
#load "deviceLanguageTaint.cmo";;
#load "client_interpreter.cmo";;
#load "server_interpreter.cmo";;
#load "device_interpreter_taint.cmo";;
#load "webi_taint.cmo";;
#load "webi_interpreter_taint.cmo";;
#load "webi_init_taint.cmo";;
#load "webi_init_interpreter_taint.cmo";;
#load "parser_taint.cmo";;
#load "lexer_taint.cmo";;
module Lst = List
...
open Webi_init_interpreter_taint
open Parser_taint
open Lexer_taint
...
module WIT = Webi_init_interpreter_taint.WEBI_Init_Interp

module SMap = Map.Make(String)

let parse prg =
  let lp = Lexing.from_string prg in
  Parser_taint.webis Lexer_taint.token lp
...
let _ =
  ...
  let (_,conf) = WIT.M.extract (WIT.M.extract (WIT.M.extract (WIT.M.extract (W.simulateWebi_scheduler
    {W.wo = WIT.wo_florida_waterplant; _E_= T.lmk_empty;
     sink = (fun s -> match s with
                    | ("valve","waterplant") | ("alarm","waterplant") -> Sensitive
                    | _ ->  Insensitive);
     dwo = [(("sodium_dispenser","waterplant"),("sodium_sensor","waterplant"));
            (("sodium_dispenser","waterplant"),("sodium_sensor","valve"))]})
    ws)
